...
class Box:
    ...
    def __init__(self, headers:dict):
        ...
        """
        self.capabilities = self.getCapabilities(headers['wtv-capability-flags'])
        if headers['wtv-client-rom-type'] == 'JP-Fiji': # FIXME: does this appear on a real box? ask whoever the fuck owns one
            self.client = 3
        ...
        if self.client == 0 or self.client == 1:
            ...
            self.systeminfo = {'version': version, 'bootrom-version': bootrom, 'romtype': romtype, 'chip-version': chipversion}
        elif self.client == 3:
            version = headers['wtv-system-version']
            romtype = headers['wtv-client-rom-type']
